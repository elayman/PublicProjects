...
public class UserTracker extends Component {
	...
	private int ZEROBUFFER = 20;
	
	//private int averagedLength = 5;
	
	private double leftPrev = 0;
	private double rightPrev = 0;
	private double clawPrev = 0;
	...
	double[] detectGesture(int user,
		...
		Point3D R_hip_pos = jointHash.get(SkeletonJoint.RIGHT_HIP).getPosition();
		Point3D L_hip_pos = jointHash.get(SkeletonJoint.LEFT_HIP).getPosition();
		...
		if (depthLeft > maxSpeed) {
			depthLeft = maxSpeed;
		} else if ( Math.abs(depthLeft) < ZEROBUFFER ){
			depthLeft = 0;
		} else if (depthLeft < -maxSpeed) {
			depthLeft = -maxSpeed;
		}
		if (depthRight > maxSpeed) {
			depthRight = maxSpeed;
		} else if ( Math.abs(depthLeft) < ZEROBUFFER ){
			depthRight = 0;
		} else if (depthRight < -maxSpeed) {
			depthRight = -maxSpeed;
		}
		...
		double low = shoulderHeight + (headHeight-shoulderHeight)/2;
		double high = 2*headHeight-shoulderHeight;
	...
	public void drawSkeleton(Graphics g, int user) throws StatusException {
		...
		if (user == 1) {
			 ...
			 */
				...
				leftFilter = alpha*leftPrev + (1-alpha)*leftSpeed;
				rightFilter = alpha*rightPrev + (1-alpha)*rightSpeed;
				clawFilter = alpha*clawPrev + (1-alpha)*clawVal;
				
				clawPrev = clawFilter;
				leftPrev = leftFilter;
				rightPrev = rightFilter;
